...
import {ChevronLeftIcon, HeartIcon, MapPinIcon, StarIcon, } from 'react-native-heroicons/outline'
    ...
    <SafeAreaView className="flex-1 bg-white relative mt-10">
        <ScrollView className="flex-1 px-3 py-3">
            <View className="relative bg-white shadow-lg">
                ...
                <View className="absolute flex-row inset-x-0 bottom-5 justify-between px-6">
                    ...
                    <View className="px-2 py-1 rounded-md bg-teal-100">
                        <Text>{data?.length ? data?.open_now_text : "No Info"}</Text>
                    </View>
                </View>
            </View>
            ...
            <View className="mt-6">
                ...
                <View className="flex-row items-center space-x-2 mt-2">
                    <MapPinIcon size={22} color="#8C9EA6" />
        ...
        <View className=" space-y-2 mt-4 bg-gray-100 rounded-2xl px-4 py-2">
            {data?.phone && (
                ...
                <Text className="text-[#10px]">{data?.phone}</Text>
                </View>
            )}
            {data?.email && (
                <View className="items-center flex-row space-x-6">
                <FontAwesome name="envelope" size={16} color="#428288" />
                <Text className="text-[#10px]">{data?.email}</Text>
                </View>
            )}
            {data?.address && (
                <View className="items-center flex-row space-x-6">
                <FontAwesome name="map-pin" size={16} color="#428288" />
                <Text className="text-[#10px]">{data?.address}</Text>
                </View>
            )}

            <TouchableOpacity onPress={() => navigation.push("BookingScreen", data)}>
